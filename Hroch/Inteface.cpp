...
SymbolicRegression::Config GetConfig(const solver_params &sp) noexcept
{
	return SymbolicRegression::Config{
		...
		sp.clip_min,
		sp.clip_max,
		{sp.init_const_min, sp.init_const_max, sp.init_predefined_const_prob, sp.init_predefined_const_set ? std::vector<double>{sp.init_predefined_const_set, sp.init_predefined_const_set + sp.init_predefined_const_count} : std::vector<double>{}},
		{sp.input_size,
		 sp.const_size,
		 sp.min_code_size,
		 sp.max_code_size}};
